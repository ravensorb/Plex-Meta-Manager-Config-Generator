...
from pmm_cfg_gen.utils.plexutils import isPMMItem, _cleanTitle
from pmm_cfg_gen.utils.fileutils import writeFile, formatLibraryItemPath
...
class PlexLibraryProcessor:
    ...
    def __init__(self) -> None:
        ...
        self.templateManager = TemplateManager(globalSettingsMgr.settings.templates.getTemplateRootPath())
    ...
    def process(self):
        ...
        if globalSettingsMgr.settings.plex.libraries is None:
            self._logger.warn("No plex libraries defined to process.  Please check your configuration")
            return
    ...
    def connectToServer(self):      
        self._logger.info("Connection to plex server: {}".format(globalSettingsMgr.settings.plex.serverUrl))

        self.__session = requests.Session()
        self.__session.verify = False
        self.plexServer = PlexServer(globalSettingsMgr.settings.plex.serverUrl, globalSettingsMgr.settings.plex.token, session=self.__session)
    ...
    def _loadLibrary(self, libraryName: str) -> LibrarySection:
        ...
        self.pathLibrary = formatLibraryItemPath(globalSettingsMgr.settings.output, self.plexLibrary)
        self.pathLibrary.mkdir(parents=True, exist_ok=True)

        self._logger.debug("Library Path: '{}'".format(self.pathLibrary))

        tplFiles = globalSettingsMgr.settings.templates.metadata.getByItemType("library")

        fileNameJson = Path(self.pathLibrary, "{}.json".format(libraryName))

        if tplFiles.jsonFileName is not None and globalSettingsMgr.settings.generate.enableJson:
            self.templateManager.renderAndSave(tplFiles.jsonFileName, fileNameJson, {"library": self.plexLibrary})
    ...
    def _addItemToThePosterDbSearchCache(self, item):
        ...
        tpdbEntry = { 'title': item.title, 'searchUrl': generateTpDbUrl(item), 'ids': ids }
    ...
    def _processLibrary(self, libraryName: str):
        ...
        for collection in collections:
            if not isPMMItem(collection) and collection.childCount > 0: 
                try:
                    ...
                    self._logger.exception("Error Processing Collection: {}".format(collection.title))
            else:
        ...
        for item in items:
            if not isPMMItem(item) and len(item.collections) == 0: 
                ...
                self._logger.info("Skipping Metdata for: {} ({})".format(item.title, item.year))  
        

        self._saveThePosterDbSeachCache(Path(self.pathLibrary, "theposterdbsearch.json").resolve())             
    ...
    def _processCollection(self, itemTitle, item):
        ...
        tplFiles = globalSettingsMgr.settings.templates.collections.getByItemType(self.plexLibrary.type)
        ...
        if not os.path.exists(fileName) and tplFiles.pmmFileName is not None:
            self.templateManager.renderAndSave(tplFiles.pmmFileName, fileName, tplArgs={"item": item})
        ...
        if tplFiles.jsonFileName is not None and globalSettingsMgr.settings.generate.enableJson:
            self.templateManager.renderAndSave(tplFiles.jsonFileName, fileNameJson, tplArgs={"item": item})
    ...
    def _processMetadata(self, itemTitle, items):
        ...
        tplFiles = globalSettingsMgr.settings.templates.metadata.getByItemType(self.plexLibrary.type)

        if not os.path.exists(fileName) and tplFiles.pmmFileName is not None:
            self.templateManager.renderAndSave(tplFiles.pmmFileName, fileName, tplArgs={"items": itemsWithExtras})
        # elif os.path.exists(fileName):
        #     # Do we want to try to merge here?
        #     pass
        else:
        ...
        if tplFiles.jsonFileName is not None and globalSettingsMgr.settings.generate.enableJson:
            self.templateManager.renderAndSave(tplFiles.jsonFileName, fileNameJson, tplArgs={"items": itemsWithExtras})
