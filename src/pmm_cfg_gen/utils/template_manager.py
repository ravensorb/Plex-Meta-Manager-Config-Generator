...
def generateTpDbUrl(item, baseUrl = None) -> str:
    ...
    if "guids" in item.__dict__:
        ...
        if "tmdb" in ids.keys(): urlParms.update( { "tmdb_id": ids["tmdb"] } )
        if "imdb" in ids.keys(): urlParms.update( { "imdb_id": ids["imdb"] } )
        if "tvdb" in ids.keys(): urlParms.update( { "tvdb_id": ids["tvdb"] } )
...
class TemplateManager:
    ...
    def __init__(self, templatePath : str | Path) -> None:
        ...
        self._logger.debug("Initializing Template Environment.  Template Path: '{}'".format(templatePath))
    ...
    def renderAndSave(self, templateName : str | Path, fileName : str | Path, tplArgs : dict):
        self._logger.debug("Render data using template '{}' and save to '{}'".format(templateName, fileName))
    ...
    def __getTemplate(self, templateName : str | Path):
        ...
        if not templateName in self.__cachedTemplates.keys():
            ...
            self._logger.debug("Retrieving template from cache: {}".format(templateName))
