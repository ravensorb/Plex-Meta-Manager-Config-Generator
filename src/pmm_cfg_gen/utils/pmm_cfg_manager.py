...
class PlexMetaManager:
    ...
    def mergeCollection(self, itemTitle : str, item):
        self._logger.info("Merging Updates into Collection Item: '{}'".format(itemTitle))
        ...
        if os.path.exists(fileName):
            with open(fileName, "r") as fp:
                data = ruamel.yaml.load(fp, ruamel.yaml.RoundTripLoader, preserve_quotes=True)
                ...
                for it in data[0]:
                    ...
                    data["collections"][it].yaml_set_comment_before_after_key("template", after=format(generateTpDbUrl(item)), after_indent=2)
        ...
        if os.path.exists(fileName):
            with open(fileName, "r") as fp:
                ...
                for it in data["metadata"]:
                    data["metadata"][it].yaml_set_comment_before_after_key("url_poster", after=format(generateTpDbUrl(item)), after_indent=2)
                    data["metadata"][it].yaml_set_comment_before_after_key("url_poster", after="IDs: ", after_indent=2)        
