...
def isPMMItem(item : PlexPartialObject):
    for label in item.labels:
        if label.tag in [ "Decade", "Emmy Awards", "Golden Globes Awards", "Top Actors", "Top Directors","Oscars Winners Awards" ]:
            logging.getLogger("pmm_cfg_gen").debug("isPMMItem Found: {} - {}".format(item.title, label.tag))
            return True
